...
from user.views import CustomLoginView, SignUpView, PostLogoutView, CustomPasswordChangeView, verify_email_view, \
    SendEmailVerificationView, SampleView
...
urlpatterns = [
    ...
    path('verification/email/', SendEmailVerificationView.as_view(), name='send_email_verification'),
    path('verify/email/<uidb64>/<token>/', verify_email_view, name='verify_email'),

    path('test/rs/', SampleView.as_view())
]
