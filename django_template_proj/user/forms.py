...
class CustomUserCreationForm(UserCreationForm):
    ...
    def save(self, commit=True):
        user = super().save()
        self._post_save(user)
        return user

    def _post_save(self, user):
        from user.tasks import send_email_verification
        send_email_verification(user, self.request_site)
...
class SendEmailVerification(forms.Form):
    ...
    def _post_clean(self):
        ...
        try:
            user = EmailUser.objects.get(email=self.cleaned_data['email'])
